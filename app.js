...
function renderResults(plan, userData) {
    ...
    // Generate Exercises (Dynamic Search Links)
    const exerciseCards = plan.exercisePlan.selectedExercises.map((ex, index) => {
        // PRIORITY: Use the search link generated by the AI Engine
        const linkUrl = ex.videoUrl || `https://www.youtube.com/results?search_query=${encodeURIComponent(ex.name + ' exercise')}`;

        // PRIORITY: Use the generic thumbnail if provided, else fallback to a safe placeholder
        const thumbUrl = ex.thumbnailUrl || 'https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?w=600&q=80';
